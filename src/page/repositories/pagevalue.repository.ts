...
@Injectable()
export class PageValueRepository
  extends GeneralRepository
  implements IRepository
{
  constructor(
    @InjectModel('PageValue')
    private readonly pageValueModel: Model<PageValue>,
    ...
    super();
    this.setModel(pageValueModel);
    this.setMainPart('PageValue');
  ...
  processOptionForList(info?: RequestListDTO): object {
    ...
    for (const filter of info.filters) {
      ...
      if (filter.field == 'idpage') {
        const tOr = [];
        const values = filter.values;
        if (Array.isArray(values)) {
          for (const value of values) {
            if (!value) continue;
            tOr.push({ idpage: this.getParsedId(value) });
          }
        }
        if (tOr.length) {
          tAnd.push({ $or: tOr });
  ...
  async deleteByIdType(id: string): Promise<any> {
    return await this.pageValueModel.deleteOne({ idpage: id });
  }
  async findOneSortBy(input: string): Promise<any> {
    return await this.pageValueModel.findOne().sort(input).exec();
  }
}
