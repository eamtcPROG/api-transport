...
@Injectable()
export class VideoValueRepository extends GeneralRepository implements IRepository
{
  constructor(
    @InjectModel('VideoValue')
    private readonly videoValueModel: Model<VideoValue>,
    ...
    super();
    this.setModel(videoValueModel);
    this.setMainPart('VideoValue');
  ...
  async populateObj(obj: any, populate?: RequestPopulateDTO): Promise<any> {
    ...
    if (populate.populates.includes('idlanguage')){
  ...
  processOptionForList(info?: RequestListDTO): object {
    ...
    for (const filter of info.filters) {
      ...
      if (filter.field == 'idvideo') {
        const tOr = [];
        const values = filter.values;
        if (Array.isArray(values)) {
          for (const value of values) {
            if (!value) continue;
            tOr.push({ idvideo: this.getParsedId(value) });
          }
        }
        if (tOr.length) {
          tAnd.push({ $or: tOr });
  ...
  async deleteByIdType(id: string): Promise<any> {
    return await this.videoValueModel.deleteOne({ idvideo: id });
  }
  async findOneSortBy(input: string): Promise<any> {
    return await this.videoValueModel.findOne().sort(input).exec();
  }
}
