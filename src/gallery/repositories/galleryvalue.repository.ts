...
@Injectable()
export class GalleryValueRepository
  extends GeneralRepository
  implements IRepository
{
  constructor(
    @InjectModel('GalleryValue')
    private readonly galleryValueModel: Model<GalleryValue>,
    ...
    super();
    this.setModel(galleryValueModel);
    this.setMainPart('GalleryValue');
  ...
  processOptionForList(info?: RequestListDTO): object {
    ...
    for (const filter of info.filters) {
      ...
      if (filter.field == 'idgallery') {
        const tOr = [];
        const values = filter.values;
        if (Array.isArray(values)) {
          for (const value of values) {
            if (!value) continue;
            tOr.push({ idgallery: this.getParsedId(value) });
          }
        }
        if (tOr.length) {
          tAnd.push({ $or: tOr });
  ...
  async deleteByIdType(id: string): Promise<any> {
    return await this.galleryValueModel.deleteOne({ idgallery: id });
  }
  async findOneSortBy(input: string): Promise<any> {
    return await this.galleryValueModel.findOne().sort(input).exec();
  }
}
